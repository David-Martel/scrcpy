            ...
            if line.strip() and not line.startswith("*"):
                ...
                if match:
                    ...
                    try:
                        ...
                        if verbose:
                            print(
                                f"Error getting SDK version for {serial}: {e}",
                                file=sys.stderr,
                            )  # Verbose error to stderr
        return devices
    except FileNotFoundError:
        print(
            "Error: adb not found. Make sure it's in your PATH.", file=sys.stderr
        )  # Error to stderr
        return []
...
def launch_scrcpy(
    ...
    try:
        ...
        print(
            "Error: scrcpy not found. Make sure it's installed and in your PATH.",
            file=sys.stderr,
        )
    except subprocess.CalledProcessError as e:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-n", "--no-audio", action="store_true", help="Disable audio forwarding."
    )
    parser.add_argument(
        "-k",
        "--keyboard-mode",
        choices=["default", "uhid"],
        default="default",
        help="Keyboard mode to use. Default or UHID",
    )
    ...
    parser.add_argument(
        ...
        "--list-devices",
        action="store_true",
        help="List available devices and options.",
    )
    ...
    if args.list_devices:
        ...
        if devices:
            for device in devices:
                ...
                print(
                    f"  Audio Forwarding Possible: {'Yes' if device['audio_possible'] else 'No'}"
                )
                print("-" * 20)
        else:
    ...
    if devices:
        if args.verbose:
            for device in devices:
                print(
                    f"Found device: {device['full_info']} (Audio: {'Possible' if device['audio_possible'] else 'Not Possible'})"
                )
        launch_scrcpy(
            args.serial,
            args.video_codec,
